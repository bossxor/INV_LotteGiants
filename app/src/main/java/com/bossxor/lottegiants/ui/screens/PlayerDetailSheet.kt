...
@Composable
private fun PlayerFace(d: PlayerDetail) {
    val urls = remember(d.playerCode, d.photoUrl) {
        buildList {
            if (d.photoUrl.isNotBlank()) add(d.photoUrl)
            addAll(playerPhotoCandidates(d.playerCode).filter { it != d.photoUrl })
        }.distinct()
    }
    var idx by remember(d.playerCode, d.photoUrl) { mutableIntStateOf(0) }
    Box(
        Modifier
            .size(72.dp)
