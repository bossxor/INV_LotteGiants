...
- 경기 전 카운트다운
- 새로고침은 컴팩트는 **우하단**, 와이드는 우상단 오버레이. 누르면 금색으로 바뀌고 강제 fetch (1.3.77). 실패해도 마지막 스코어를 다시 그린다 (1.3.85). 컴팩트 **경기 전**은 순위→로고→선발→시간→`장소 · 잔여`(가운데) (2.0.13). **선발·잔여 글자 확대, 구장명 표시** (2.0.1).
