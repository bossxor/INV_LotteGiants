...
- **알림 / Now Bar / Wear OS**
  ...
  - Now Bar는 설정한 **경기 시작 N분 전**(30분~4시간, 30분 단위, 기본 2시간)부터. 경기 중에는 항상. 끝나면 밀어 지울 수 있음
  - Now Bar 칩은 **라이브 바**·**점수만**이고 **경기 중**일 때만 승격. **상세 알림**은 커스텀 카드라 칩과 동시에 못 씀. 시스템에서 사직스코어 라이브 알림(Now Bar) 허용 필요
  - 알림을 누르면 해당 화면으로 바로 감. 득점·홈런·역전은 **중계**, 라인업은 **라인업**, 등말소는 **엔트리**, 매직/트래직은 **순위**
...
- **설정**: 즐겨찾기 선수, 알림 종류별 토글, 표시 모드, Now Bar 시작 시각(30분 단위), 테마, 자동 업데이트
