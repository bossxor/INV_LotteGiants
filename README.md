...
- 상단 **히어로**는 로고·점수·이닝을 한 덩어리로 표시. 오늘 롯데 경기가 없으면 **최근 결과 + 다음 경기**를 한눈에 (1.3.87)
- **우천중단**은 `우천중단 (19:50 예정)`처럼 사유·재개 시각 표기 (취소와 구분, LIVE 유지)
- 경기 중: 다이아몬드 · BSO · 투수/타자
- 오늘 경기는 가로 카드, 상세는 **프리뷰 · 라인업 · 요약 · 중계 · 기록** 탭
- 다른 팀 경기도 상세 확인 가능 (메인·위젯·알림은 **설정한 내 팀** 기준, 기본값 롯데)
- 하단 **라이브** 탭을 누르면 내 팀 경기로 복귀

### 중계 · 요약

| 탭 | 내용 |
|----|------|
| **중계** | 이닝 칩 + 아웃카운트·타석 묶음 문자중계. 이 탭을 열 때 지난 이닝을 합친다 (1.3.87) |
...
- LIVE 경기는 **`LIVE · 7회초`**처럼 이닝 표시 (내 팀·타팀 동일)
- 결과에서 경기를 열었다가 뒤로가면 **결과 탭**으로 복귀
- DH · 순위 · 취소 사유 표시
- 예정 경기: **홈=파랑, 원정=빨강**
...
- 상단은 세 덩어리: **레이스 요약**(순위·최근 5·매직/트래직·페이스) · **잔여 상대**(홈/원정 + 팀 로고) · **다음 5경기**(로고·시각·구장·홈/원정)
- **KBO 순위표** 칸: 경기 · 승-무-패 · **연속** · 승률 · 게임차. 한 팀을 누르면 그 팀 기준 게임차
- 그 아래 **내 팀 시즌 트렌드** · 타이틀 순위 · **등록/말소 공시** (엔트리·타이틀은 보고 있는 팀 기준)
- 엔트리 화면: 일자별 **등록·말소** 모두 표시, 변동이 있던 날짜 하이라이트
- 레이스 카드 공유 가능

### 하단 탭

**라이브 · 결과 · 선수 · 순위 · 설정** — 이름을 항상 표시. 선택된 탭은 볼드·롯데 레드.

### 설정

- **내 팀** 10구단 전환 (스냅샷·알림·위젯·테마·홈 아이콘)
- 알림 종류별 토글 · 표시 모드 · 알림 표시 시작(30분 단위)
- 득점·역전 알림 진동 on/off (1.3.87)
- 테마(라이트/다크/시스템) · 자동 업데이트 (앱 시작과 동시에 확인)
- 즐겨찾기·알림 설정 JSON 백업/복원. 삼성 잠자기 앱에서 빼기 (1.3.86)
...
| 득점권 | **중계** 탭 | `주자 1,2루 · 타석 장두성` |
| 라인업 | **라인업** 탭 | — |
| 엔트리 변동 | **엔트리** 화면 | 등록·말소, 또는 `오늘 등말소 변화 없음` |
| 매직/트래직 | **순위** 탭 | `12 → 11 · 롯데, 두산 5:3 승` |

- **역전**: 지고 있던 팀이 앞설 때만 (`3:4 → 5:4`). 동점 선취(`0:0 → 0:1`)는 역전이 아님
- 홈런, 투수 교체(`KT - 스키모토`), 라인업, 취소, 즐겨찾기 **타석·등판·등말소** 등
...
- **펼친 알림**: 로고–점수–다이아몬드–점수–로고 / 투수·**세로 BSO**·타자 / 승률 바. B/S/O 왼쪽 정렬로 투수·타자 이름 공간 확보
...
- 경기 전 카운트다운
- 새로고침은 오른쪽 위 오버레이 버튼. 누르면 금색으로 바뀌고 강제 fetch (1.3.77). 실패해도 마지막 스코어를 다시 그린다 (1.3.85). 컴팩트 **경기 전**은 순위→로고→선발→시간→장소→잔여 순, 로고 줄 end 패딩 없이 가운데 정렬 (2.0.11). **선발·잔여 글자 확대, 구장명 표시** (2.0.1).
...
LIVE 스냅샷은 현재 이닝 중계만 받고, **중계 탭을 열 때** 지난 이닝을 합친다. 루타 승률·하이라이트는 **25초**에 한 번 (1.3.87).
...
`env.ps1`이 `L:\`를 저장소에 붙이면 이후는 `L:\`에서 돌린다. `subst L:`은 **재부팅·절전 후 사라지므로** 그때는 `env.ps1`을 다시 실행한다.
...
| **2.0.2** | 설정에서 내 팀 전환. 스냅샷·알림·위젯·테마·히스토리·홈 아이콘이 선택한 팀을 따라감 (기본값 롯데). S26에 `latest` 덮어쓰기 설치 |
| **2.0.1** | 위젯 선발·잔여 글자 확대, 구장명 표시 (컴팩트·라이브·종료 포함). S26에 `latest` 덮어쓰기 설치 |
| **2.0.0** | 앱 이름 집관. 10구단 구장 배지 런처 아이콘·activity-alias. 위젯 추가 화면 중립화. Wear 모듈 삭제. `versionCode` 2000. S26에 `latest` 덮어쓰기 설치 |
...
| **1.3.81** | 잔여 상대·다음 5경기에 로고 |
| **1.3.80** | 폰 APK에서 Wear 번들 제거. 순위표에 연속 칸 |
