...
현재 버전: **`1.3.29`** (`versionCode` **1036**). `app/build.gradle.kts`에서만 올립니다.
...
- **홈 위젯** / **Now Bar** / **이벤트 알림** / **Wear OS**
  - Now Bar 지원 폰(One UI 라이브 알림)에서는 진행 중 점수가 잠금화면·상태 칩에 표시됨. 설정에서 라이브 바 + Now Bar 허용
...
| 필드 | 설명 | 현재 |
|------|------|------|
| `versionName` | 사용자에게 보이는 버전 | `1.3.29` |
| `versionCode` | 업데이트 비교용 정수 (배포마다 +1) | `1036` |
