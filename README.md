...
현재 버전: **`1.3.27`** (`versionCode` **1034**). `app/build.gradle.kts`에서만 올립니다.

## 기능

- **라이브**: 점수, 이닝, S/B/O, 루상·주자 타순, 투수/타자, 라인업, 프리뷰·요약·중계·기록 탭. 다른 팀 경기도 상세 확인(메인·위젯·알림은 항상 롯데). 하단 **라이브** 탭은 롯데 경기로 돌아옴
...
| 필드 | 설명 | 현재 |
|------|------|------|
| `versionName` | 사용자에게 보이는 버전 | `1.3.27` |
| `versionCode` | 업데이트 비교용 정수 (배포마다 +1) | `1034` |
