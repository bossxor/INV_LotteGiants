...
승리확률은 네이버 중계 → 루타 → 추정 순으로 소스를 고르고, 0:0 초반에도 API가 주는 값(예: 40:60)은 그대로 표시한다. `1`/`99` 같은 퍼센트 정수를 잘못 읽어 100:99로 보이던 파싱 버그는 1.3.61에서 수정했다.
...
| 모드 | 설명 |
|------|------|
| **라이브 바** | Now Bar 칩 + 펼친 알림 |
| **상세 알림** | 커스텀 카드 (칩과 동시 사용 불가) |
| **점수만** | 점수 중심 간단 표시 |
...
1. 앱 시작 시 `update.json`을 읽어 `versionCode`가 더 크면 APK 다운로드
2. **앱 내부 PackageInstaller**로 자기 자신을 갱신 (외부 파일 앱으로 열지 않음)
3. 이 앱이 설치 주체가 되면 확인 없이 끝날 수 있고, 아니면 시스템 확인 한 번
