    ...
    for ($d = 1; $d -le 28; $d++) {
        ...
        try {
            ...
            foreach ($g in $r.game) {
                ...
                if ($id -ge 1) { $ids[$id] = $g.CANCEL_SC_NM }
            }
        } catch {}
    }
}
$lines = $ids.GetEnumerator() | Sort-Object Name | ForEach-Object { "$($_.Name)=$($_.Value)" }
[System.IO.File]::WriteAllLines("L:\kbo_cancel_map.txt", $lines, [System.Text.UTF8Encoding]::new($false))
