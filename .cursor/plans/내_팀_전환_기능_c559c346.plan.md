...
5. 기기에서 롯데→두산 전환 후 히어로·순위·위젯·알림·홈 아이콘(`집관 잠실`) 확인, 다시 롯데

유닛 범위: `involvesTeam`, 워터마크 매핑, 알림 문구, DataStore 삭제 키 목록, `LauncherIcon.normalize`, ScoreAlerts/MagicNumber 팀명 파라미터
...
아래는 본체 없이도 돌아가지만, 같이 넣으면 전환 체감이 좋아진다. **넣을 항목만 고른다.**

| id | 후보 | 내용 | 난이도 |
|----|------|------|--------|
| A | 위젯 `"롯데"` → `focusName()` | BeforeWide/LiveWide/EndedWide 팀명 하드코딩 일괄 교체 (로고·색은 본체 widget todo) | low |
| B | 온보딩·설정 안내 문구 | `"집관 안내"`에 내 팀 변경 한 줄 추가 | low |
| C | 결과 탭 기본 필터 = 내 팀 | `resultsTeamCode` 초기값을 `myTeamCode`로 (지금은 LT 고정) | low |
| D | 즐겨찾기 팀 전환 안내 | 팀 바꾸면 즐겨찾기 선수가 다른 팀일 수 있다고 확인 다이얼로그에 적기 | low |
| E | Keubo 팀카드 실존 확인 | 10구단 slug로 team-card 한 번 호출해 실패 시 빈 카드 UI | med |
| F | Wear 빈 폴더 정리 | 디스크에 남은 `wear/` 잔여 제거 | low |
| G | 설정 백업에 런처 아이콘 동기화 | import 후에도 `LauncherIcon.request` 호출 | low |
